...
public class MainActivity extends AppCompatActivity {
    ...
    MediaPlayer musicSadPlayer;
    MediaPlayer musicFunPlayer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pausemusicBtn = findViewById(R.id.pausePlay_music);
        ...
        pausemusicBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(musicFunPlayer != null)
                {
                    if (musicFunPlayer.isPlaying()) {
                        musicFunPlayer.pause();
                    } else {
                        musicFunPlayer.start();
                    }
                }
                if(musicSadPlayer != null)
                {
                    if (musicSadPlayer.isPlaying()) {
                        musicSadPlayer.pause();
                    } else {
                        musicSadPlayer.start();
                    }
                }
            }
        });
        ...
        stopmusicBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(musicFunPlayer != null) musicFunPlayer.stop();
                if(musicSadPlayer != null) musicSadPlayer.stop();
            }
        });
    ...
    private void classifyEmotions(Bitmap imageBitmap, int faceId) {
        ...
        Toast.makeText(MainActivity.this, reversedKeys.get(0), Toast.LENGTH_LONG).show();
        if(reversedKeys.get(0).equals("нейтрально") )
        {
            musicSadPlayer = MediaPlayer.create(this, R.raw.sad1);
            musicSadPlayer.start();
        }
        if(reversedKeys.get(0).equals("грусть") )
        {
            musicSadPlayer = MediaPlayer.create(this, R.raw.sad2);
            musicSadPlayer.start();
        }
        if(reversedKeys.get(0).equals("злость") )
        {
            musicSadPlayer = MediaPlayer.create(this, R.raw.sad3);
            musicSadPlayer.start();
        }
        if(reversedKeys.get(0).equals("счастье") )
        {
            musicFunPlayer = MediaPlayer.create(this, R.raw.monako);
            musicFunPlayer.start();
        }
    }
